...
g = polyfit(x,y,1)
print('Для I=1mA',g[0]*b/(1*1e-3))
y=poly1d(g)
x=linspace(0,x[-1],1000)

plot(x,y(x),label=r'$I=1\text{ мА}$')
plot(df['I']*773/1e4,df['E'],'r.',label='_nolegend_')
...
y=array(df['E']) # В Вольтах
g = polyfit(x,y,1)
print('Для I=4mA',g[0]*b/(4*1e-3))
y=poly1d(g)
x=linspace(0,x[-1],1000)

plot(x,y(x),label=r'$I=4\text{ мА}$')
plot(773*df['I']/1e4,df['E'],'r.',label='_nolegend_')
